...
* **2026-01-24**：**核心升级**：新增 `RoleBank`角色 与 `AdvancedDialogue` 多角色配音节点，新增 `ScriptProcessor`，支持 `[情感标签]` 与 `[pause:停顿]` 自动解析，并增加**Seed 随机种子**、**输出模式(合并/分段)**控制、**音频转文本**功能。
...
| **max_new_tokens** | 1024 | 生成最大长度。CPU模式下会被强制限制以防卡死。 |
| **temperature** | 0.7 | 采样温度。越高情感越丰富（但也越不可控），越低越稳定。 |
